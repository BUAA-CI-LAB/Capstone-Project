...
static int decode_tokens_in_batches(
    ...
    for (int i = 0; i < (int) tokens.size(); i += BATCH_SIZE) {
        ...
        if (start_pos + i + cur_batch_size >= DEFAULT_CONTEXT_SIZE - OVERFLOW_HEADROOM) {
            ...
            shift_context();
            batch_start_pos = current_position;
        }

        // Add tokens to the batch with proper positions
        for (int j = 0; j < cur_batch_size; j++) {
            const llama_token token_id = tokens[i + j];
            const llama_pos position = batch_start_pos + j;
        ...
        batch_start_pos += cur_batch_size;
    }
    return 0;
}
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_prefillHistorySliding(
        JNIEnv *env,
        jobject /*unused*/,
        jobjectArray jmsgs
) {
    ...
    const int max_tokens = DEFAULT_CONTEXT_SIZE - OVERFLOW_HEADROOM;
    int total_tokens = 0;

    int num_msgs = env->GetArrayLength(jmsgs);
    std::vector<std::pair<std::string, std::string>> recent_msgs;

    for (int i = num_msgs - 1; i >= 0; i--) {
        jstring jmsg = (jstring) env->GetObjectArrayElement(jmsgs, i);
        const char* msg = env->GetStringUTFChars(jmsg, nullptr);

        std::string role = (i % 2 == 0) ? ROLE_USER : ROLE_ASSISTANT;
        std::string formatted = chat_add_and_format(role, msg);

        auto tokens = common_tokenize(g_context, formatted, true, true);

        if (total_tokens + tokens.size() > max_tokens) {
            env->ReleaseStringUTFChars(jmsg, msg);
            LOGi("prefill history: exceed max tokens, skip");
            break;
        }

        total_tokens += tokens.size();
        recent_msgs.push_back({role, formatted});
        env->ReleaseStringUTFChars(jmsg, msg);
    }

    std::reverse(recent_msgs.begin(), recent_msgs.end());
    int pos = 0;
    for (auto &msg_pair : recent_msgs) {
        auto tokens = common_tokenize(g_context, msg_pair.second, true, true);
        if (decode_tokens_in_batches(g_context, g_batch, tokens, pos)) {
            return 2;
        }
        pos += tokens.size();
    }
    ...
    system_prompt_position = 0; // 如果没有系统 prompt
    return 0;
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_clearModelMemory(
        JNIEnv *env,
        jobject /*unused*/) {
    reset_long_term_states();
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_processUserPrompt(
    ...
    LOGi("current position: %d", current_position);
