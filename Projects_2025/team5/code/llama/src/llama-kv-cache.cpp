...
//wy_new_add
void llama_kv_cache::keep_last(uint32_t keep_last_n) {
    const uint32_t n_tokens = this->get_size();

    if (n_tokens <= keep_last_n) {
        return;
    }

    const uint32_t n_drop = n_tokens - keep_last_n;
...
void llama_kv_cache::apply_h2o_eviction(float alpha, float beta, size_t window) {
    if (keys.empty()) return;

    // 1. 计算每个 token 的重要性
    std::vector<float> importance(keys.size(), 0.0f);
    for (size_t i = 0; i < keys.size(); i++) {
        importance[i] = compute_importance(i, alpha, beta, window);
    }

    // 2. 根据 importance 排序，保留 top-k
    size_t keep_count = std::max(size_t(1), keys.size() / 2); // 示例: 保留一半
    std::vector<size_t> idx_sorted(keys.size());
    ...
    // 3. 淘汰低 importance token
    for (size_t i = keep_count; i < idx_sorted.size(); i++) {
        size_t idx = idx_sorted[i];
        keys[idx].clear();
        values[idx].clear();
        if (!frequencies.empty()) frequencies[idx] = 0.0f;
    }

    // 4. 压缩数组或重排
    // keys.erase(std::remove_if(keys.begin(), keys.end(), [](auto &k){ return k.empty(); }), keys.end());
    // values.erase(std::remove_if(values.begin(), values.end(), [](auto &v){ return v.empty(); }), values.end());
}

float llama_kv_cache::compute_importance(size_t idx, float alpha, float beta, size_t window) const {
    // 1. 最近 token 权重
    size_t start = (idx >= window) ? (idx - window) : 0;
    float recency_score = alpha * (1.0f - float(idx - start) / float(window));

    // 2. 高频 token 权重
    float freq_score = 0.0f;
    if (!frequencies.empty()) {
        freq_score = beta * frequencies[idx]; // frequencies[idx] 已归一化或自定义
    }
