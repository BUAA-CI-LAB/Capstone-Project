...
llama_context::llama_context(
    ...
    // wy_new_add
    cparams.snap_kv_on = true;
    cparams.snap_kv_n = 32;
    cparams.h2o_on = false;      // 新增开关
    cparams.h2o_max_kv = 32;   // H2O 最大 KV 数量
    if (cparams.use_h2o) {
        token_scores.reserve(cparams.h2o_max_kv);
        n_kv = 0;
    }
...
bool llama_context::memory_update(bool optimize) {
    ...
    // wy_new_add===== SnapKV: 在 init_update 之前裁剪 KV =====
    if (cparams.snapkv_tokens > 0) {
        // memory 实际上就是 kv_self
        static_cast<llama_kv_cache *>(memory.get())
            ->keep_last(cparams.snapkv_tokens);
    }
    if (cparams.h2o_on) {
        static_cast<llama_kv_cache *>(memory.get())
            ->apply_h2o_eviction(cparams.h2o_alpha,
                                 cparams.h2o_beta,
                                 cparams.h2o_window);
    }
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    memory_update(false);
    // wy_new_add===== 在这里插入 SnapKV =====
    if (cparams.snap_kv_on) {
        kv_self->keep_last(cparams.snap_kv_n);  // keep_n 可以从配置里读
    }
    if (cparams.h2o_on) {
        kv_self->apply_h2o_eviction(cparams.h2o_alpha,
                                    cparams.h2o_beta,
                                    cparams.h2o_window);
    }
    // ====================================
