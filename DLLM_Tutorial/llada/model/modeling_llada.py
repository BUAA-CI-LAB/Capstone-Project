...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        if use_sparse:
            ...
            num_external_to_keep = min(int(0.5 * scores_external.shape[0]), scores_external.shape[0])
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position,
                                        use_sparse=use_sparse, current_block_indices=current_block_indices)
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model.forward(
            ...
            current_block_indices=current_block_indices
        )
        # import pdb; pdb.set_trace()
